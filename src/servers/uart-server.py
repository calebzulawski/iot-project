...
parser.add_argument('--blocksize', type=int, default=2000, help='transmission block size')
...
try:
    with open(args.mp3_file, 'rb') as f, serial.Serial(args.serial, args.baud, timeout=1) as ser:
        f.seek(2500)
        s = f.read(args.blocksize)
        ser.write(s)
        while True:
            # if s == "":
                # break
